...
subroutine gr(xx,yy,zz,natoms,boxsize,g,nhis)
    implicit none
    ...
    do i=1,natoms-1
        do j=i+1,natoms
            ...
            !if(r .lt. m) m = int(r/delg)*boxsize/nhis
            if(r .lt. m) m = r
            if(r .lt. 2.0) write(*,*) r
                ig = int(r/delg)
...
program hw1
    ...
    call rescale_vol(x,y,z,natoms,boxsize,rho)
    write(*,*) "Rescaled box size to", boxsize

    nhis = 250
    call gr(x,y,z,natoms,boxsize,g,nhis)
    ...
    write(*,*) "step, time, epot, ekin, etotal, Ecohesive, temperature"
    !write(*,*) "step, epot, temperature"
    ! MD Simulation
    ...
    do i=1, ceiling(10.0/timestep)
        ...
        !write(*,*) "Finished step",i,"of",10.0/timestep
        epot = sum(e1)
        !write(*,*) "Epot:", epot
        !write(*,*) "Cohesive energy:", epot/natoms
        !write(*,*)
        call calc_temp(temperature,natoms,vx,vy,vz)
        ekin = 3.0/2.0*natoms*kb*temperature
        write(*,"(I4,6F9.3)") i, i*timestep*eps*1000, epot*eps, ekin*eps, (epot+ekin)*eps, epot*eps/natoms, temperature/kb*eps
        !write(*,*) i, epot*eps, temperature/kb*eps
    enddo
